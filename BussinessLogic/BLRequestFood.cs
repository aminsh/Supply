...
{
    public class BLRequestFood :BLBase<RequestFood>
    {
        public override void OnSubmitEntity(RequestFood entity, System.Data.EntityState state, Dictionary<string, object> originalValues)
        {
        ...
        public void Do()
        {
            var total = Context.RequestFoods.GroupBy(g => g.Section).Select(g => new
                {
                    Key = g.Key.ID,
                    Total = 
                    Context.RequestFoods
                    .Where(rf => rf.Section.FullPathID.StartsWith(g.Key.FullPathID))
                    .SelectMany(rf => rf.RequestDetailFoods)
                        .Sum(rfd => rfd.TotalPrice)
                });
        }
    }
}

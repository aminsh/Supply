...
function (logger, app, router,com) {
    var activate = function (param) {
        ...
        baseColumns.first(function(item) {
    ...
    var setTitle = function(requestType) {
        var fixedExp = 'مرور درخواست ';
        switch (requestType) {
        case 'Service':
            {
                title(fixedExp + 'خدمات');
            }
            break;
        case 'Food':
            {
                title(fixedExp + 'مواد غذایی');
            }
            break;
            case 'Vehicle':
            {
                title(fixedExp + 'نقلیه');
            }
            break;
        default:
            {
                title('Parameter is not exists ....');
            }
        }
    };
    ...
    var pagerType = ko.observable(pagerTypes.first(function(item) {return item.key === 'canAdded';}));
    var pageSize = ko.observable(pager.pageSize());
    var requestType = ko.observable(''); //Good,Service,Food,Vehicle,Ticket
    ...
    var dictionary = [
        ...
        { key: 'ItemService', value: 'کالا' },
        { key: 'ItemFood', value: 'کالا' },
        { key: 'ItemVehicle', value: 'کالا' },
    ...
    var baseColumns = [
        ...
        { name: ko.observable('Item'), title: ko.observable('کالا'), isActive: ko.observable(false), type: ko.observable('group') },
        ...
        keys.foreach(function (key) {
            params[key].subscribe(function () {
                ...
                if (key === 'DateFrom') {
                    ...
                    obj.displayValue = getDisplayName(key,params[key]());
                    obj.dataType = 'int';
                }
    ...
    var statusParam = ko.observableArray([
        { name: 'pendding', title: 'درحال انجام' },
        { name: 'cancel', title: 'لغو شده' },
        { name: 'done', title: 'انجام شده' },
        { name: 'closed', title: 'بسته شده' }
    ]);
    var getDisplayName = function(entityName, obj) {
        if (entityName === 'SectionAssistance')
            entityName = 'Section';
        var displaycolumn = helper.modelInfo.getDisplayColumnByEntity(entityName);
        if(displaycolumn==='')
            throw new Error('Dispaly name not found ...');

        var value = '';

        displaycolumn.split(',').foreach(function(item) {
            value = value == '' ? obj[item] : ' ' + obj[item];
        });
    ...
    var activateState = function(index) {
        if (index < 0)
            throw new Error('Index for stateHistory array not Service ...');
    ...
    var loadRequest = function (item) {
        ...
        var q = context.query('Request' + requestType() + 'Xs');
    ...
    var runQuery = function (state) {
        ...
        if (state == undefined) {
            ...
            q = context.query('Request' + requestType() + 'Grouping');
    ...
    var getColumnValue = function(parent, data) {
        var value = parent[data.name()];
        if (typeof value == 'object')
            return getDisplayName(data.name(), value);
